...
// Parse mesh trace file to discover actual HWMP path for a specific flow
MeshPathAnalysis ParseMeshTraceForFlow(Ipv4Address srcIP, Ipv4Address dstIP, 
                                        std::string traceFile, uint32_t nMeshAPs)
{
    MeshPathAnalysis result;
    result.pathFound = false;
    
    std::ifstream trace(traceFile);
    if (!trace.is_open()) {
        std::cout << "  Warning: Could not open mesh trace file " << traceFile << std::endl;
        return result;
    }
    
    // Convert IPs to strings for matching
    std::ostringstream srcStream, dstStream;
    srcIP.Print(srcStream);
    dstIP.Print(dstStream);
    std::string srcIPStr = srcStream.str();
    std::string dstIPStr = dstStream.str();
    
    // Track forwarding events per node
    std::map<uint32_t, MeshHopInfo> nodeStats;
    std::set<uint32_t> nodesInvolved;
    
    std::string line;
    while (std::getline(trace, line)) {
        // Look for lines with our flow's IPs
        if (line.find(srcIPStr) == std::string::npos || line.find(dstIPStr) == std::string::npos) {
            continue;
        }
        
        // Extract node ID from "/NodeList/X/"
        size_t nodeListPos = line.find("/NodeList/");
        if (nodeListPos == std::string::npos) continue;
        
        size_t nodeIdStart = nodeListPos + 10; // length of "/NodeList/"
        size_t nodeIdEnd = line.find("/", nodeIdStart);
        if (nodeIdEnd == std::string::npos) continue;
        
        uint32_t nodeId = std::stoul(line.substr(nodeIdStart, nodeIdEnd - nodeIdStart));
        
        // Skip if not a mesh AP (only track APs 0 to nMeshAPs-1)
        if (nodeId >= nMeshAPs) continue;
        
        // Initialize node stats if needed
        if (nodeStats.find(nodeId) == nodeStats.end()) {
            nodeStats[nodeId].apIndex = nodeId;
            nodesInvolved.insert(nodeId);
        }
        
        // Count events: tx (send/forward), rx (receive), drop
        if (line.find(" t ") != std::string::npos || line.find("Enqueue") != std::string::npos) {
            nodeStats[nodeId].txCount++;
        } else if (line.find(" r ") != std::string::npos || line.find("Dequeue") != std::string::npos) {
            nodeStats[nodeId].rxCount++;
        } else if (line.find(" d ") != std::string::npos || line.find("Drop") != std::string::npos) {
            nodeStats[nodeId].dropCount++;
        }
    }
    trace.close();
    
    if (nodesInvolved.empty()) {
        return result;
    }
    
    // Build path sequence (sort by AP index as approximation)
    // In reality, HWMP may use different order, but this gives us the involved APs
    result.pathSequence.assign(nodesInvolved.begin(), nodesInvolved.end());
    std::sort(result.pathSequence.begin(), result.pathSequence.end());
    
    // Calculate hop PDR
    for (auto& nodePair : nodeStats) {
        MeshHopInfo& info = nodePair.second;
        if (info.rxCount > 0) {
            info.hopPDR = (info.txCount * 100.0) / info.rxCount;
        }
    }
    
    result.hopStats = nodeStats;
    result.pathFound = true;
    
    // Calculate totals
    if (!result.pathSequence.empty()) {
        uint32_t sourceAP = result.pathSequence.front();
        uint32_t destAP = result.pathSequence.back();
        
        if (nodeStats.find(sourceAP) != nodeStats.end()) {
            result.totalMeshPackets = nodeStats[sourceAP].txCount;
        }
        if (nodeStats.find(destAP) != nodeStats.end()) {
            result.totalMeshDelivered = nodeStats[destAP].rxCount;
        }
    }
...
void AnalyzeAllFlows(uint32_t nMeshAPs, std::string traceFile)
{
    ...
    for (auto& flowPair : g_trackedFlows) {
        ...
        // ========== MESH-LEVEL PATH ANALYSIS ==========
        std::cout << "\n╔══════════════════════════════════════════════════════╗" << std::endl;
        std::cout << "║       MESH PATH ANALYSIS (from trace file)          ║" << std::endl;
        std::cout << "╚══════════════════════════════════════════════════════╝" << std::endl;
        
        MeshPathAnalysis meshPath = ParseMeshTraceForFlow(flow.flowId.source, 
                                                            flow.flowId.destination, 
                                                            traceFile, 
                                                            nMeshAPs);
        
        if (meshPath.pathFound) {
            std::cout << "\n  Actual HWMP Path: ";
            for (size_t i = 0; i < meshPath.pathSequence.size(); i++) {
                std::cout << "AP" << meshPath.pathSequence[i];
                if (i < meshPath.pathSequence.size() - 1) std::cout << " → ";
            }
            std::cout << " (" << (meshPath.pathSequence.size() - 1) << " mesh hops)" << std::endl;
            
            std::cout << "\n  Hop-by-Hop Mesh Forwarding:" << std::endl;
            for (size_t i = 0; i < meshPath.pathSequence.size(); i++) {
                uint32_t apIdx = meshPath.pathSequence[i];
                if (meshPath.hopStats.find(apIdx) != meshPath.hopStats.end()) {
                    const MeshHopInfo& info = meshPath.hopStats.at(apIdx);
                    
                    std::cout << "    Hop " << (i+1) << ": AP" << apIdx;
                    if (i == 0) std::cout << " (Source)";
                    else if (i == meshPath.pathSequence.size() - 1) std::cout << " (Destination)";
                    std::cout << std::endl;
                    
                    if (i == 0) {
                        std::cout << "      Mesh TX: " << info.txCount << " packets" << std::endl;
                    } else if (i == meshPath.pathSequence.size() - 1) {
                        std::cout << "      Mesh RX: " << info.rxCount << " packets" << std::endl;
                    } else {
                        std::cout << "      Mesh RX: " << info.rxCount 
                                  << ", TX: " << info.txCount 
                                  << ", DROP: " << info.dropCount << std::endl;
                        if (info.rxCount > 0) {
                            double hopPDR = (info.txCount * 100.0) / info.rxCount;
                            std::cout << "      Hop PDR: " << std::fixed << std::setprecision(2) << hopPDR << "%";
                            if (hopPDR < 95.0) std::cout << " ⚠️";
                            if (info.dropCount > 10) std::cout << " (HIGH DROP!)";
                            std::cout << std::endl;
                        }
                    }
                }
            ...
            if (meshPath.totalMeshPackets > 0) {
                double meshE2EPDR = (meshPath.totalMeshDelivered * 100.0) / meshPath.totalMeshPackets;
                std::cout << "\n  Mesh E2E Delivery: " << std::fixed << std::setprecision(2) << meshE2EPDR << "% (" 
                          << meshPath.totalMeshDelivered << "/" << meshPath.totalMeshPackets << " packets)";
                if (meshE2EPDR >= 95.0) std::cout << " ✅";
                else if (meshE2EPDR >= 80.0) std::cout << " ⚠️";
                else std::cout << " ❌";
                std::cout << std::endl;
            }
        } else {
            std::cout << "\n  No mesh path found (direct connection or trace unavailable)" << std::endl;
        }
...
int main(int argc, char *argv[])
{
    ...
    // Use list routing to avoid global routing conflicts with mesh
    ...
    internet.Install(meshAPNodes);
    internet.Install(internetNode);
    internet.Install(externalServer);
    internet.Install(staNodes);
    
    std::cout << "Installed TCP/IP stack with static routing on all nodes (including external server)" << std::endl;
    ...
    std::cout << "Mesh APs: Configured routes to external network and other STAs" << std::endl;
    ...
    std::cout << "Static routes configured: All STAs can reach external server" << std::endl;
