...
const std::string OUTPUT_DIR = "wifi_mesh_outputs";
...
int main(int argc, char **argv) {
  ...
  phy.EnablePcapAll(OUTPUT_DIR + "/wifi_mesh_playfield_rw_pcap", true);

  // ASCII traces (.tr)
  phy.EnableAsciiAll(OUTPUT_DIR + "/wifi_mesh_playfield_ascii_traces");
  ...
  AnimationInterface anim(
      OUTPUT_DIR + "/netanim-wifi-mesh-playfield-rw.xml");
  ...
  // IPv4 L3 ASCII tracing (emit packets at IP layer to ASCII file)
  {
    ...
    Ptr<OutputStreamWrapper> ipStream = ascii.CreateFileStream(OUTPUT_DIR + "/ipv4-l3.tr");
  ...
  monitor->SerializeToXmlFile(
      OUTPUT_DIR + "/flowmon-wifi-mesh-playfield-rw.xml", true, true);
  Simulator::Destroy();
  return 0;
}