...
static const std::string kPcapPrefix = "5g_playfield_rw_pcap";
static const std::string kAsciiTracesPrefix = "5g_playfield_ascii_traces";
static const std::string kNetAnimFile = "netanim-5g-playfield-rw.xml";
static const std::string kFlowmonFile = "flowmon-5g-playfield-rw.xml";

// Write an ASCII grid of node and building positions to 5g_outputs/position_grid.txt
...
int main(int argc, char **argv) {
  ...
  // Nodes: 10 5G UEs; we'll pin index 0 as Sayed, last as Sadia
  NodeContainer nodes;
  nodes.Create(nNodes);
  ...
  // Group 1: RandomWalk2d (nodes 1-3) - Increased speed to 10 m/s for 5G
  ...
  group1Mob.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
    ...
    "Speed", StringValue("ns3::ConstantRandomVariable[Constant=10.0]")); // 10 m/s for 5G
  ...
  group2Mob.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
    ...
    "Speed", StringValue("ns3::ConstantRandomVariable[Constant=8.0]")); // 8 m/s
  ...
  group3Mob.SetMobilityModel("ns3::RandomWalk2dMobilityModel",
    ...
    "Speed", StringValue("ns3::ConstantRandomVariable[Constant=12.0]")); // 12 m/s
  ...
  // 5G Wi‑Fi channel/PHY with high Tx power + extended range for 5G characteristics
  ...
  // Use range cap appropriate for 5G - higher range than WiFi mesh
  // Diagonal spacing ≈ 62.8 m for 10 nodes; set MaxRange to ~120 m for 5G
  chan.AddPropagationLoss("ns3::RangePropagationLossModel", "MaxRange",
                          DoubleValue(120.0));

  YansWifiPhyHelper phy;
  phy.SetChannel(chan.Create());
  phy.Set("TxPowerStart", DoubleValue(20.0)); // Higher power for 5G
  phy.Set("TxPowerEnd", DoubleValue(20.0));
  phy.Set("RxNoiseFigure", DoubleValue(5.0)); // Lower noise figure for 5G
  // Ensure outputs directory exists at project root
  std::system(("mkdir -p " + kOutDir).c_str());

  // 5G Wi-Fi (802.11ax) mesh
  // NOTE: This simulation uses WiFi mesh as a proxy for 5G NR
  // Real 5G would require gNodeB (5G base station) and 5G Core Network
  // ns-3 doesn't have dedicated 5G NR modules, so we simulate 5G characteristics
  // using WiFi mesh with 5G-appropriate parameters (high speed, mesh topology)
  MeshHelper mesh = MeshHelper::Default();
  mesh.SetStackInstaller("ns3::Dot11sStack");
  mesh.SetSpreadInterfaceChannels(MeshHelper::SPREAD_CHANNELS);
  mesh.SetMacType("RandomStart", TimeValue(Seconds(0.1))); // Faster startup for 5G
  mesh.SetNumberOfInterfaces(1); // Ensure one interface for mesh
  ...
  // Internet + IPs with routing
  InternetStackHelper internet;
  
  // Add OLSR routing protocol for mesh networks
  OlsrHelper olsr;
  ...
  // Enable global routing for mesh networks
  ...
  udpClientA.SetConstantRate(DataRate("8Mbps"), 1200); // Higher rate for 5G
  ...
  udpClientB.SetConstantRate(DataRate("8Mbps"), 1200); // Higher rate for 5G
  ...
  for (uint32_t i = 1; i < nNodes - 1; ++i) {
    ...
    iotToSayed.SetAttribute("MaxPackets", UintegerValue(300)); // More packets for 5G
    iotToSayed.SetAttribute("Interval", TimeValue(Seconds(1.0)));  // More frequent for 5G
    iotToSayed.SetAttribute("PacketSize", UintegerValue(150)); // Larger packets for 5G
  ...
  // NetAnim: label ends and color all nodes
  ...
  for (uint32_t i = 0; i < nNodes; ++i) {
    if (i == 0) {
      // Sayed (node 0) - Blue
      anim.UpdateNodeDescription(nodes.Get(i), "Sayed");
      anim.UpdateNodeColor(nodes.Get(i), 0, 150, 255);
    } else if (i == nNodes - 1) {
      // Sadia (node 9) - Orange
      anim.UpdateNodeDescription(nodes.Get(i), "Sadia");
      anim.UpdateNodeColor(nodes.Get(i), 255, 120, 0);
    } else {
      // Middle nodes - Different colors by group
      std::string nodeName = "UE" + std::to_string(i);
      anim.UpdateNodeDescription(nodes.Get(i), nodeName.c_str());
      
      if (i <= 3) {
        // Group 1 (nodes 1-3) - Green (high speed)
        anim.UpdateNodeColor(nodes.Get(i), 0, 255, 0);
      } else if (i <= 6) {
        // Group 2 (nodes 4-6) - Yellow (medium speed)
        anim.UpdateNodeColor(nodes.Get(i), 255, 255, 0);
      } else {
        // Group 3 (nodes 7-8) - Red (very high speed)
