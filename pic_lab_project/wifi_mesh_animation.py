#!/usr/bin/env python3
"""
WiFi Mesh Network Animation with Moving Buildings
Creates an animated visualization of the WiFi mesh simulation with moving buildings.
"""
...
class WiFiMeshAnimator:
    def __init__(self, output_dir="wifi_mesh_outputs"):
        ...
        self.node_trails = [[] for _ in range(self.n_nodes)]
        
    def _generate_node_positions(self):
        """Generate node positions along diagonal"""
        positions = []
        for i in range(self.n_nodes):
            if i == 0:  # Sayed
                positions.append((0.0, 0.0))
            elif i == self.n_nodes - 1:  # Sadia
                positions.append((self.field_size, self.field_size))
            else:  # Middle nodes
                frac = i / (self.n_nodes - 1)
    ...
    def _simulate_node_movement(self, time):
        """Simulate RandomWalk2d movement for middle nodes (1-8)"""
        # Sayed (node 0) and Sadia (node 9) are static
        # Only nodes 1-8 move with RandomWalk2d
        ...
        for i in range(1, self.n_nodes - 1):  # nodes 1-8
            ...
            if np.random.random() < 0.3:  # 30% chance to change direction
                ...
                distance = self.node_speed * 0.5  # Half the speed for smoother movement
                ...
                x_min, x_max, y_min, y_max = self.node_bounds
    ...
    def create_animation(self):
        ...
        print("Creating WiFi mesh animation with moving buildings...")
        
        # Set up the figure and axis
        fig, ax = plt.subplots(figsize=(12, 10))
        
        # Initialize plot elements
        node_scatter = ax.scatter([], [], c='blue', s=100, label='Mobile Mesh Nodes', zorder=4)
        ...
        # Trail lines for mobile nodes
        trail_lines = []
        for i in range(1, self.n_nodes - 1):  # Only for mobile nodes
        ...
        ax.set_title('WiFi Mesh Network with Moving Buildings')
        ...
        def animate(frame):
            ...
            # Simulate node movement for middle nodes (1-8)
            self._simulate_node_movement(time)
            
            # Update node positions
            node_x = [pos[0] for pos in self.node_positions[1:-1]]
            node_y = [pos[1] for pos in self.node_positions[1:-1]]
            
            # Update scatter plots
            node_scatter.set_offsets(list(zip(node_x, node_y)))
            sayed_scatter.set_offsets([self.node_positions[0]])
            sadia_scatter.set_offsets([self.node_positions[-1]])
            
            # Update trails for mobile nodes
            for i, line in enumerate(trail_lines):
                node_idx = i + 1  # nodes 1-8
                self.node_trails[node_idx].append(self.node_positions[node_idx])
                
                # Keep only last 20 positions for trail
                if len(self.node_trails[node_idx]) > 20:
                    self.node_trails[node_idx] = self.node_trails[node_idx][-20:]
                
                if len(self.node_trails[node_idx]) > 1:
                    trail_x = [pos[0] for pos in self.node_trails[node_idx]]
                    trail_y = [pos[1] for pos in self.node_trails[node_idx]]
            ...
            return [node_scatter, sayed_scatter, sadia_scatter, time_text] + mobile_rects
        ...
        output_file = os.path.join(self.output_dir, 'wifi_mesh_animation.gif')
        ...
        try:
            mp4_file = os.path.join(self.output_dir, 'wifi_mesh_animation.mp4')
    ...
    def create_static_plot(self):
        ...
        # Plot nodes
        node_x = [pos[0] for pos in self.node_positions]
        node_y = [pos[1] for pos in self.node_positions]
        
        ax.scatter(node_x[1:-1], node_y[1:-1], c='blue', s=100, label='Mesh Nodes', zorder=4)
        ax.scatter(node_x[0], node_y[0], c='cyan', s=150, marker='s', label='Sayed', zorder=5)
        ax.scatter(node_x[-1], node_y[-1], c='orange', s=150, marker='s', label='Sadia', zorder=5)
        ...
        # Add connections (simplified mesh connections)
        ...
        # Label special nodes
        ax.annotate('Sayed', (node_x[0], node_y[0]), 
                   xytext=(10, 10), textcoords='offset points', 
                   fontsize=10, fontweight='bold')
        ax.annotate('Sadia', (node_x[-1], node_y[-1]), 
                   xytext=(10, 10), textcoords='offset points', 
                   fontsize=10, fontweight='bold')
        ...
        ax.set_xlabel('X Position (m)')
        ax.set_ylabel('Y Position (m)')
        ax.set_title('WiFi Mesh Network Layout')
        ax.legend()
        ax.grid(True, alpha=0.3)
        ax.set_aspect('equal')
        
        plt.tight_layout()
        output_file = os.path.join(self.output_dir, 'wifi_mesh_static_layout.png')
...
def main():
    """Main function"""
    print("WiFi Mesh Network Animator")
    print("=" * 40)
    
    # Create animator
    animator = WiFiMeshAnimator()
