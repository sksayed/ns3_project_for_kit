...
TR_DIR = "5g_outputs"
TR_GLOB = os.path.join(TR_DIR, "5g_playfield_ascii_traces-*.tr")
...
def plot_rate_distribution(df: pd.DataFrame, out_dir: str):
    ...
    plt.title('5G PHY rate usage (all frames)')
    plt.ylabel('Frames')
    plt.xlabel('Rate')
    plt.tight_layout()
...
def plot_mac_throughput(df: pd.DataFrame, out_dir: str):
    ...
    plt.title('5G MAC throughput over time (QOSDATA)')
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def plot_udp_port_throughput(df: pd.DataFrame, out_dir: str):
    ...
    plt.title('5G UDP traffic by destination port (MAC level)')
    plt.tight_layout()
...
def plot_most_common_path(paths_df: pd.DataFrame, out_dir: str, title_suffix: str = ''):
    ...
    plt.title(f'Most common 5G path {title_suffix}: ' + path_str)
    plt.tight_layout()
...
def animate_paths_time(df: pd.DataFrame, out_dir: str, src_ip: str = '10.0.0.1', dst_ip: str = '10.0.0.10', dst_port: int = 5000):
    ...
    title = ax.set_title(f'Time-ordered 5G path animation UDP {dst_port} {src_ip}→{dst_ip}')
...
def main():
    ...
    print("Saved: traces_parsed.csv, tr_summary.csv, tr_by_rate.csv, tr_udp_ports.csv and PNG plots in 5g_outputs")
